...
export default class VisualAidService extends Service {
    ...
    targetCache;
    toolDisposers = [];
    agentToolCleanups = new Map();
    sessionData = new Map();
    storageDir;
    inFlight = new Map();
    projected = new WeakSet();
    constructor(ctx, config) {
        ...
        ctx.on('session/event', (_session, event) => {
            ...
            if (event.type === 'user/message' || event.type === 'tool/result') {
                ...
                if (hasImage)
                    void this.settleDescriptions(_session, undefined, undefined);
            }
        });
        ...
        ctx.inject(['commands', 'settings'], (commandCtx) => {
            commandCtx.commands.register({
                ...
                handler: async (invocation) => {
                    ...
                    if (!on && !off)
                        throw new Error('/visual-aid expects "on" or "off"');
                    ...
                    if (agent !== undefined)
                        this.refreshAgentTools(agent);
                    return { kind: 'success', text: on ? 'visual aid enabled for this session' : 'visual aid disabled for this session' };
                },
            });
        });
    ...
    patchResolveModelInfo() {
        ...
        llm.resolveModelInfo = async (provider, model, signal) => {
            ...
            if (!this.current.masqueradeMultimodal)
                return info;
            ...
            if (provider === this.current.provider && model === this.current.model)
                return info;
    ...
    invalidateTarget() { this.targetCache = undefined; }
    resolveTarget(session, signal) {
        ...
        if (this.targetCache?.key === key)
            return this.targetCache.promise;
        const promise = (async () => {
    ...
    inheritSessionData(session) {
        ...
        if (parentId === undefined)
            return;
        if (this.sessionData.has(session.id))
            return;
        if (existsSync(this.sessionFilePathFor(session.id)))
            return;
    ...
    recordOperation(session, type, data) {
        ...
        const operation = { type, time: Date.now(), data };
    ...
    dataFor(session) {
        ...
        const entry = this.sessionData.get(session.id);
        if (entry === undefined)
            throw new Error('visual-aid: session data is missing after load');
        return entry;
    }
    qasFor(session) {
        return this.dataFor(session).qas
            .filter(q => q.status === 'answered' && q.answer !== undefined)
            .map(q => ({ imageNos: [...q.imageNos].sort((a, b) => a - b), question: q.question, answer: q.answer }));
    }
    recordsFor(session) {
    ...
    async describeOne(session, record, target, slot) {
        try {
            ...
            const result = await this.runVisionTextWithRetry(target, session.id, [createUserMessage({ content: [{ type: 'text', text: DESCRIBE_PROMPT }, { type: 'image', attachment: { attachmentId: record.attachmentId, mediaType: record.mediaType, bytes: record.bytes, width: record.width, height: record.height, ...record.name === undefined ? {} : { name: record.name } } }], source: { kind: 'plugin', plugin: NAME } })], DESCRIBE_SYSTEM, this.effectiveDescribeMaxTokens(target), this.current.timeoutMs, undefined);
            const data = this.dataFor(session);
            data.imageRecords.set(record.imageNo, {
                ...record,
                status: 'described',
            ...
            if (result.usage?.inputTokens !== undefined)
                data.currentDescribeInput = describeTotalInput;
            if (result.usage?.outputTokens !== undefined)
                data.currentDescribeOutput = result.usage.outputTokens;
    ...
    refreshTool() {
        for (const dispose of this.toolDisposers)
            dispose();
        this.toolDisposers = [];
        if (!this.current.enabled || this.current.provider.length === 0 || this.current.model.length === 0)
            return;
    ...
    refreshAgentTools(agent) {
        ...
        if (enabled === this.current.enabled)
            return;
        if (enabled) {
    ...
    visualReadImageDefinition() {
        return defineTool({
            ...
            output: {
                ...
                render: (_args, value) => [{ type: 'text', text: `<path>${value.path}</path>\n<type>image</type>\n<content>${value.image.mediaType} image, ${value.image.width}x${value.image.height} px</content>` }, { type: 'image', attachment: { attachmentId: value.image.attachmentId, mediaType: value.image.mediaType, bytes: value.image.bytes, width: value.image.width, height: value.image.height, ...value.image.name === undefined ? {} : { name: value.image.name } } }],
            },
    ...
    async readImage(exec, filePath) {
        ...
        if (mediaType === undefined)
            throw new Error(`visual_read_image only accepts PNG/JPEG/WebP/GIF paths: ${filePath}`);
        const attachments = this.ctx.get('attachments');
        if (attachments === undefined)
            throw new Error('visual_read_image requires an attachment service');
        const fs = this.ctx.get('fs');
        if (fs === undefined)
            throw new Error('visual_read_image requires a filesystem service');
    ...
    async executeViewImage(exec, rawIds, question) {
        ...
        if (session === undefined)
            throw new Error('view_image requires a session tool call');
        if (!this.enabledFor(session))
            throw new Error('visual-aid is disabled for this session');
        const target = await this.resolveTarget(session, exec.signal);
        if (target === undefined) {
            if (this.hasConfiguredModel(session))
                throw new Error(`view_image: configured vision model ${this.configuredModelLabel(session)} is unavailable or does not accept image input`);
        ...
        const parse = (raw) => {
            ...
            if (match !== null && byNo.has(Number(match[1])))
                return Number(match[1]);
            const found = byAttachment.get(raw.trim());
            if (found !== undefined)
                return found.imageNo;
        ...
        if (contextWindow === undefined)
            throw new Error('view_image: visual model declares no context window');
        ...
        try {
            ...
            if (result.usage?.inputTokens !== undefined)
                data.currentContextTokens = queryTotalInput;
            if (result.usage?.inputTokens !== undefined)
                data.currentQueryInput = queryTotalInput;
            if (result.usage?.outputTokens !== undefined)
                data.currentQueryOutput = result.usage.outputTokens;
    ...
    effectiveMaxTokens(target) {
        ...
        if (configured !== undefined && configured !== DEFAULT_MAX_TOKENS)
            return configured;
    ...
    effectiveDescribeMaxTokens(target) {
        ...
        if (configured !== undefined && configured > 512)
            return configured;
    ...
    async runVisionText(target, sessionId, messages, system, maxTokens, timeoutMs, signal) {
        ...
        try {
            ...
            const options = deepFreeze({
                ...
                maxTokens,
                sessionId,
                ...this.current.reasoningEffort === undefined ? {} : { reasoningEffort: this.current.reasoningEffort },
            ...
            if (text.length === 0)
                throw new Error(`visual-aid produced no text on ${target.provider}/${target.model}`);
